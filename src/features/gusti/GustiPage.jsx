...
export function GustiPage({ prefs, onToggleLike, onResetPrefs }) {
  ...
  const Stat = ({emoji,n,label,color}) => (
    <div style={{flex:1,background:"#fff",borderRadius:10,border:"1.5px solid #E7EDE2",padding:"12px 8px",textAlign:"center"}}>
      <div style={{fontSize:20}}>{emoji}</div>
      <div style={{fontSize:20,fontWeight:800,color,fontFamily:"monospace",marginTop:2}}>{n}</div>
      <div style={{fontSize:9,color:"#9DB1A2",fontWeight:700,textTransform:"uppercase",letterSpacing:0.4,marginTop:2}}>{label}</div>
    </div>
  );
  ...
  return (
    <div>
      {/* Intro */}
      <div style={{background:"#EEF7F0",border:"1px solid #A9DDB8",borderRadius:10,padding:"10px 14px",marginBottom:14,fontSize:11,color:"#1F6B4A",lineHeight:1.6}}>
      ...
      {!hasAnyData ? (
        <div style={{textAlign:"center",padding:"50px 20px",color:"#9DB1A2"}}>
          <div style={{fontSize:48,marginBottom:12}}>🌱</div>
          <div style={{fontSize:14,fontWeight:700,color:"#6E8576"}}>Nessun dato ancora</div>
